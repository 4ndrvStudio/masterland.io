...
namespace FishNet.Object
{
    ...
    public abstract partial class NetworkBehaviour : MonoBehaviour
    {
        ...
        [MakePublic]
        [APIExclude]
        protected internal void Server_SendReconcileRpc<T>(uint hash, ref T lastReconcileData, T reconcileData, Channel channel)
        {
            if (!IsSpawned)
                return;
...
//#if !FISHNET_STABLE_MODE
#if DO_NOT_USE
            methodWriter.WriteDeltaReplicate<T>(replicatesHistory, offset, deltaOption);
#else
...
//#if !FISHNET_STABLE_MODE
#if DO_NOT_USE
            receivedReplicatesCount = reader.ReadDeltaReplicate<T>(lastReadReplicate, ref arrBuffer, tick);
#else
            ...
            Replicate_SendNonAuthoritative<T>(hash,
                replicatesQueue, channel);
        }
...
//#if !FISHNET_STABLE_MODE
#if DO_NOT_USE
            methodWriter.WriteDeltaReplicate<T>(replicatesQueue, redundancyCount, GetDeltaSerializeOption());
#else
...
//#if !FISHNET_STABLE_MODE
#if DO_NOT_USE
            T newData = reader.ReadDeltaReconcile<T>(lastReconciledata);
#else
